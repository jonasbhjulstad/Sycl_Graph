...
namespace Sycl_Graph::Sycl {
  ...
  sycl::event invoke_operation(Graph_t&, const Op& operation, sycl::queue& q,
    ...
    return q.submit([&](sycl::handler& h) {
      ...
      h.parallel_for(operation.N_parallel,
                     [=](sycl::id<1> id) { operation(source_acc, result_acc, id); });
    });
  }
  ...
  template <Graph_type Graph_t, Operation_type... Op>
  std::tuple<std::vector<typename Op::Result_t>...> apply_transform_operations(
      Graph_t& graph, const std::tuple<Op...>& operations, sycl::queue& q) {
    auto result_bufs = construct_operation_buffers<typename Op::Result_t...>(operations);
    ...
    if constexpr (op_0.operation_type == Operation_Buffer_Transform) {
      ...
      auto events
          = invoke_operations(graph, q, operations, std::make_pair(source_bufs, result_bufs));
    } else {
