...
struct Edge_Extract_Operation : public Operation_Base<Edge_Extract_Operation<Edge_Buffer_t, Derived>>
{
    ...
    static constexpr std::array<sycl::access_mode, 3> graph_access_modes = {sycl::access_mode::read,
...
template <Sycl_Graph::Edge_Buffer_type Edge_Buffer_t, typename Derived>
struct Edge_Inject_Operation : public Operation_Base<Edge_Inject_Operation<Edge_Buffer_t, Derived>>
{
    typedef typename Edge_Buffer_t::Edge_t Edge_t;
    typedef typename Edge_t::From_t From_t;
    typedef typename Edge_t::To_t To_t;
    typedef std::tuple<Edge_t, From_t, To_t> Accessor_Types;
    static constexpr std::array<sycl::access_mode, 3> graph_access_modes = {sycl::access_mode::read_write,
                                                                            sycl::access_mode::read,
                                                                            sycl::access_mode::read};
...
template <Sycl_Graph::Edge_Buffer_type Edge_Buffer_t, typename Derived>
struct Edge_Transform_Operation : public Operation_Base<Edge_Transform_Operation<Edge_Buffer_t, Derived>>
{
    typedef typename Edge_Buffer_t::Edge_t Edge_t;
    typedef typename Edge_t::From_t From_t;
    typedef typename Edge_t::To_t To_t;
    ...
    static constexpr std::array<sycl::access_mode, 3> graph_access_modes = {sycl::access_mode::read,
                                                                            ...
                                                                            sycl::access_mode::read};
    static constexpr sycl::access_mode target_access_mode = sycl::access_mode::write;
    void _invoke(auto &accessors, auto &custom_acc, const auto &source_acc, auto &target_acc, sycl::handler &h)
    {
        auto &edge_acc = std::get<0>(accessors);
        auto &from_acc = std::get<1>(accessors);
        auto &to_acc = std::get<2>(accessors);
        static_cast<Derived *>(this)->invoke(edge_acc, from_acc, to_acc, custom_acc, source_acc, target_acc, h);
    }
