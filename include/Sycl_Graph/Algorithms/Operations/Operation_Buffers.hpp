...
namespace Sycl_Graph::Sycl {
  ...
  auto create_target_buffer(const Graph_t& G, const Op& op) {
    ...
    if constexpr (has_Target_v<Op>)
      return std::make_shared<sycl::buffer<typename Op::Target_t>>(sycl::buffer<typename Op::Target_t>((sycl::range<1>(op.target_buffer_size(G)))));
    else
      return std::shared_ptr<sycl::buffer<Operation_Buffer_Void_t>>{};
  }
  ...
  auto create_operation_buffers(const Graph_t& G, const std::tuple<Op>& operation,
    ...
    return std::make_pair(std::make_tuple(create_source_buffer(G, std::get<0>(operation), target_buffer_prev)),
