...
namespace Sycl_Graph
{
  ...
  struct Graph
  {
    ...
    uint32_t N_vertices() const { return vertex_buf.current_size(); }
    uint32_t N_edges() const { return edge_buf.current_size(); }
    uint32_t N_vertices_max() const { return vertex_buf.max_size(); }
    ...
    template <typename T>
    auto get_edges(const std::vector<uint32_t>&& ids)
    {
