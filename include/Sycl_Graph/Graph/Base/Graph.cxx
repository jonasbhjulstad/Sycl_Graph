...
export module Base.Graph;
import Base.Graph_Types;
template <Vertex_Buffer_type _Vertex_Buffer_t, Edge_Buffer_type _Edge_Buffer_t> struct Graph {
  Graph() = default;
  ...
  template <typename T> static constexpr bool has_Edge_type
      = is_Edge_type<T>
        && Edge_Buffer_t::template is_Data_types<typename T::Connection_IDs, typename T::Data_t>;

  template <typename T> static constexpr bool has_Vertex_type
      = is_Vertex_type<T>
        && Vertex_Buffer_t::template is_Data_types<typename T::ID_t, typename T::Data_t>;
  ...
  void add_vertex(const auto&&... args) { vertex_buf.add(std::forward<decltype(args)>(args)...); }

  void add_edge(const auto&&... args) { edge_buf.add(std::forward<decltype(args)>(args)...); }

  void remove_vertex(const auto&&... args) {
    vertex_buf.remove(std::forward<decltype(args)>(args)...);
  }

  void remove_edge(const auto&&... args) { edge_buf.remove(std::forward<decltype(args)>(args)...); }
  template <typename T> auto get_edges(const std::vector<uI_t>&& ids) {
    return edge_buf.template get_edges<T>(std::forward<decltype(ids)>(ids));
  }
  template <typename T> auto get_edges() { return edge_buf.template get_edges<T>(); }
  auto get_edges() { return edge_buf.get_edges(); }
};

export template <typename T> concept Graph_type
    = requires(T t) {
