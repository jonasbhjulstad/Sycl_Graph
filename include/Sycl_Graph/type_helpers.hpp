...
struct Type_Map
{
    ...
    T get()
    {
        return values[index_of_type<U, Ts...>];
    }
};
...
struct TypeSet : TypeBase<Ts>...
{
    static constexpr std::tuple<Ts...> types{};
    template <typename T>
    constexpr auto operator+(TypeBase<T>)
    {
...
template <typename wanted_type, typename T>
struct is_one_of
{
...
struct is_one_of<wanted_type, std::tuple<Ts...>>
{
...
template <typename wanted_element_t,
          template <typename, typename...>
          typename Predicate,
          typename tuple_type,
          std::size_t... i>
constexpr auto get_type_t(const tuple_type &t, std::index_sequence<i...>)
{
    return std::tuple_cat(
        extract_type<i, typename std::tuple_element<i, tuple_type>::type, wanted_element_t, Predicate>::do_extract_type(
            t)...);
}

template <typename... wanted_element_t, template <typename, typename...> typename Predicate, typename... types>
constexpr auto predicate_get_type(const std::tuple<types...> &t)
{
    return get_type_t<std::tuple<wanted_element_t...>, Predicate>(t, std::make_index_sequence<sizeof...(types)>());
}

template <typename... wanted_element_t, typename... types>
constexpr auto get_type(const std::tuple<types...> &t)
{
    return get_type_t<std::tuple<wanted_element_t...>, is_one_of>(t, std::make_index_sequence<sizeof...(types)>());
}

template <typename... wanted_element_t, typename... types>
constexpr auto get_type_with_edge(const std::tuple<types...> &t)
{
    return get_type_t<std::tuple<wanted_element_t...>, is_vertex_of>(t, std::make_index_sequence<sizeof...(types)>());
}

template <typename... wanted_element_t, typename... types>
constexpr auto get_type_with_vertex(const std::tuple<types...> &t)
{
    return get_type_t<std::tuple<wanted_element_t...>, is_edge_of>(t, std::make_index_sequence<sizeof...(types)>());
}
    ...
    return std::apply(
        [&](auto... elem) {
            return std::make_tuple(
                get_type_t<std::tuple<decltype(elem)>, Predicate>(t, std::make_index_sequence<sizeof...(Ts)>())...);
        },
        unique_elems);
}

template <typename... Ts>
constexpr auto type_sort(const std::tuple<Ts...> &t)
{
    return predicate_sort<is_one_of>(t);
}

template <typename... Ts>
constexpr auto edge_sort(const std::tuple<Ts...> &t)
{
    return predicate_sort<is_edge_of>(t);
}

template <typename... Ts>
constexpr auto vertex_sort(const std::tuple<Ts...> &t)
{
...
auto tuple_zip(Head &&head, Tail &&...tail)
{
    ...
    return detail::tuple_zip_impl<Head, Tail...>(std::forward<Head>(head),
...
constexpr auto drop_first_tuple_elem(const Tuple &tuple)
{
...
constexpr decltype(auto) multi_apply_imp_1(std::index_sequence<Is...>, F &&f, std::tuple<Tuples...> &&t)
{
    ...
    return multi_apply_imp_2(std::index_sequence<indices.first[Is]...>{},
                             std::index_sequence<indices.second[Is]...>{},
                             std::forward<F>(f),
                             std::move(t));
}

template <typename F, class... Tuples>
...
struct EmptyTupleGenerator {
    using type = decltype(std::tuple_cat(std::declval<typename EmptyTupleGenerator<N-1>::type>(),
                                         std::tuple<>()));
};
...
template <std::size_t N>
using EmptyTuple = typename EmptyTupleGenerator<N>::type;


template <std::size_t N, typename T>
struct UniformTupleGenerator {
    using type = decltype(std::tuple_cat(std::declval<typename UniformTupleGenerator<N-1, T>::type>(),
                                         T{}));
};

template <typename T>
struct UniformTupleGenerator<0, T> {
    using type = T;
};
