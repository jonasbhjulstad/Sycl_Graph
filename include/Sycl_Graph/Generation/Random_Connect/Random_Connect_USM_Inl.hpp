...
template <typename RNG>
  ...
  struct Merge_Edge_Vectors {
    Merge_Edge_Vectors(Edge_t* e_acc,
                        const uint32_t* count_acc,
                       uint32_t* Ntot_acc, uint32_t Npw)
  ...
  template <typename RNG>
  sycl::event random_connect(sycl::queue &q, const uint32_t* from,
                             uint32_t* &to, RNG* &rngs, const float p,
  ...
  std::vector<Edge_t> random_connect(sycl::queue &q, const std::vector<uint32_t> &from,
    ...
    uint32_t* from_buf(from.data(), sycl::range<1>(from.size()));
    uint32_t* to_buf(to.data(), sycl::range<1>(to.size()));
