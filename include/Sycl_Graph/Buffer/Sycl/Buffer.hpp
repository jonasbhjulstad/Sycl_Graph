...
struct Buffer_Accessor
{
    ...
    Buffer_Accessor(std::shared_ptr<sycl::buffer<Ds, 1>> &...bufs, sycl::handler &h, sycl::property_list props = {})
        : accessors(sycl::accessor<Ds, 1, Mode>(*bufs, h, props)...)
    {
    }

    static Buffer_Accessor<Mode, Ds...> make_accessor(std::tuple<sycl::buffer<Ds, 1>...> &bufs,
                                                      sycl::handler &h,
                                                      sycl::property_list props = {})
    {
        return std::apply([&h, &props](auto &...bufs) { return Buffer_Accessor<Mode, Ds...>(bufs..., h, props); },
                          bufs);
    }
...
{
    Buffer_Accessor(sycl::buffer<D, 1> &buf, sycl::handler &h, sycl::property_list props = {})
...
struct Buffer
{
    ...
    sycl::queue &q;
    std::tuple<std::shared_ptr<sycl::buffer<Ds, 1>>...> bufs;
          ...
          q(q), logger(spdlog::basic_logger_mt(logger_filename, logger_filename + ".log"))
    {
        ...
        {
        return Buffer_Accessor<Mode, D_subset...>(std::get<std::shared_ptr<sycl::buffer<D_subset, 1>>>(bufs)..., h);
        }
