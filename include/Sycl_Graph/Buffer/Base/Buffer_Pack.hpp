...
namespace Sycl_Graph {
  template <Sycl_Graph::Buffer_type... Bs> struct Buffer_Pack
  {
    ...
    typedef std::tuple<std::shared_ptr<Bs>...> Buffer_t;
    typedef std::tuple<typename Bs::Data_t...> Data_t;
    static constexpr uint32_t N_buffers = sizeof...(Bs);
    Buffer_Pack() = default;
    Buffer_Pack(const std::shared_ptr<Bs> &...buffers) : buffers(std::make_tuple(buffers...)) {}
    Buffer_Pack(const std::tuple<std::shared_ptr<Bs>...> &buffers) : buffers(buffers) {}
    Buffer_Pack(const Buffer_Pack &other) : buffers(other.buffers) {}

    typedef Buffer_Pack<std::shared_ptr<Bs>...> This_t;
    Buffer_t buffers;

    static constexpr uint32_t invalid_id = std::numeric_limits<uint32_t>::max();

    template <typename T> static constexpr bool is_Buffer_type = has_type<std::shared_ptr<T>, Buffer_t>::value;
