...
export module Base.Buffer;

export template <typename T> concept Buffer_type = true;

export template <Buffer_type... Bs> struct Buffer_Pack {
  ...
  template <typename T> static constexpr bool is_Data_type
      = (has_type<T, typename Bs::Data_t>::value && ...);
...
export template <typename T> concept Buffer_Pack_type = Buffer_type<T> && requires(T t) {
                                                                            t.buffers;
                                                                            // T::Buffer_t;
                                                                          };
export template <typename T> concept Edge_Buffer_type = true;

export template <typename T> concept Vertex_Buffer_type = true;