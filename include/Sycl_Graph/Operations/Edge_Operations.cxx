export module Operations.Edge;
#include <Sycl_Graph/Common/common.hpp>
import Operations.Types;
template <Edge_Buffer_type Edge_Buffer_t, typename Derived> export struct Edge_Extract_Operation {
  ...
  void _invoke(auto& accessors, auto& target_acc, sycl::handler& h) const {
    auto& edge_acc = std::get<0>(accessors);
    auto& from_acc = std::get<1>(accessors);
    auto& to_acc = std::get<2>(accessors);
    static_cast<const Derived*>(this)->invoke(edge_acc, from_acc, to_acc, target_acc, h);
  }
...
export template <typename T> concept Edge_Extract_Operation_type
    = requires {
        ...
        { T::graph_access_modes } -> std::same_as<std::array<sycl::access_mode, 3>>;
        { T::target_access_mode } -> std::same_as<sycl::access_mode>;
        { T::_invoke } -> std::same_as<void>;
      };

template <Edge_Buffer_type Edge_Buffer_t, typename Derived> export struct Edge_Inject_Operation {
  typedef typename Edge_Buffer_t::Edge_t Edge_t;
  typedef typename Edge_t::From_t From_t;
  typedef typename Edge_t::To_t To_t;
  typedef std::tuple<Edge_t, From_t, To_t> Accessor_Types;
  ...
  void _invoke(auto& accessors, const auto& source_acc, sycl::handler& h) const {
    auto& edge_acc = std::get<0>(accessors);
    auto& from_acc = std::get<1>(accessors);
    auto& to_acc = std::get<2>(accessors);
    static_cast<const Derived*>(this)->invoke(edge_acc, from_acc, to_acc, source_acc, h);
  }
...
export template <typename T> concept Edge_Inject_Operation_type
    = requires {
        typename T::Edge_Buffer_t;
        typename T::Edge_t;
        typename T::From_t;
...
template <Edge_Buffer_type Edge_Buffer_t, typename Derived> export struct Edge_Transform_Operation {
  typedef typename Edge_Buffer_t::Edge_t Edge_t;
  typedef typename Edge_t::From_t From_t;
  typedef typename Edge_t::To_t To_t;
  typedef std::tuple<Edge_t, From_t, To_t> Accessor_Types;
  static constexpr std::array<sycl::access_mode, 3> graph_access_modes
      = {sycl::access_mode::read, sycl::access_mode::read, sycl::access_mode::read};
  static constexpr sycl::access_mode target_access_mode = sycl::access_mode::write;
  void _invoke(auto& accessors, const auto& source_acc, auto& target_acc, sycl::handler& h) const {
    auto& edge_acc = std::get<0>(accessors);
    auto& from_acc = std::get<1>(accessors);
    auto& to_acc = std::get<2>(accessors);
    static_cast<const Derived*>(this)->invoke(edge_acc, from_acc, to_acc, source_acc, target_acc,
                                              h);
  }
...
export template <typename T> concept Edge_Transform_Operation_type
    = requires {
        typename T::Edge_Buffer_t;
        typename T::Edge_t;
        typename T::From_t;
